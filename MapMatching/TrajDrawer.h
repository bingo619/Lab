/* 
 * Last Updated at [2015/3/19 13:04] by wuhao
 * version 1.0.1
 * comments:增加drawOneTraj的头尾识别功能 @2015/3/19
 */
...
void TrajDrawer::drawOneTraj(Traj* traj, MapDrawer& md, Gdiplus::Color color, bool boldLine /* = false */, bool drawLines /* = true */, bool bigPoint /* = true */, bool drawTrajPts /* = true */)
{
	...
	{
		if (bigPoint)
			md.drawBigPoint(Gdiplus::Color::Red, traj->front()->lat, traj->front()->lon);
		else
			md.drawPoint(Gdiplus::Color::Red, traj->back()->lat, traj->back()->lon);
	}
	if (drawTrajPts)//画终点
	{
		if (bigPoint)
