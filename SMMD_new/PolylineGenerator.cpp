...
double PolylineGenerator::calculate(double x, double y, int i)
{
	...
	else if (k == 2 && i == 1)
	{
		...
		if (abs(dist(x, y, polyline[i + 1].x, polyline[i + 1].y)) > eps && abs(dist(x, y, polyline[i - 1].x, polyline[i - 1].y)) > eps)
		{
	...
	else
	{
		if (i == 0)
		{
			...
			if (abs(dist(x, y, polyline[i + 1].x, polyline[i + 1].y)) > eps && abs(dist(x, y, polyline[i + 2].x, polyline[i + 2].y)) > eps)
			{
...
double PolylineGenerator::getRadius()
{
	...
	if (radius == 0)
	{
		cout << "[Error]r = 0" << "pts.size = " << pts.size() << endl;
		system("pause");
	}
	return radius;
}
